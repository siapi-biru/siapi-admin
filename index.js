...
const hasEE = require('siapi/lib/utils/ee');
...
module.exports = {
  ${plugins
    .map(name => {
      const shortName = name.replace(/^siapi-plugin-/i, '');
...
async function copyAdmin(dest) {
  const adminPath = getPkgPath('siapi-admin');
...
async function createCacheDir(dir) {
  ...
  const pluginsToOverride = pluginsToCopy.reduce((acc, current) => {
    const pluginName = current.replace(/^siapi-plugin-/i, '');
  ...
  await Promise.all(
    pluginsToOverride.map(plugin =>
      copyCustomAdmin(
        ...
        path.join(cacheDir, 'plugins', `siapi-plugin-${plugin}`)
      )
    )
  );
}
...
async function watchFiles(dir, ignoreFiles = []) {
  ...
  const pluginsToWatch = appPlugins.map(plugin =>
    path.join(extensionsPath, plugin.replace(/^siapi-plugin-/i, ''), 'admin')
  );
  ...
  watcher.on('all', async (event, filePath) => {
    ...
    const packageName = isExtension ? `siapi-plugin-${pluginName}` : 'siapi-admin';
