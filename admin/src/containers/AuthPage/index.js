...
import { BaselineAlignment, auth, useQuery } from 'siapi-helper-plugin';
...
const AuthPage = ({ hasAdmin, setHasAdmin }) => {
      ...
      const getData = async () => {
        try {
          const {
            ...
            `${siapi.backendURL}/admin/registration-info?registrationToken=${registrationToken}`
          );
  ...
  const forgotPasswordRequest = async (body, requestURL) => {
    try {
      await axios({
        method: 'POST',
        url: `${siapi.backendURL}${requestURL}`,
        data: body,
