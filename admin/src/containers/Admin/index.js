...
// Components from siapi-helper-plugin
import {
  difference,
...
import { checkLatestSiapiVersion } from '../../utils';
...
import { getSiapiLatestReleaseSucceeded, setAppError } from './actions';
...
export class Admin extends React.Component {
  ...
  fetchSiapiLatestRelease = async () => {
    const {
      global: { siapiVersion },
      getSiapiLatestReleaseSucceeded,
    } = this.props;
    ...
    try {
      const {
        data: { tag_name },
      } = await axios.get('https://api.github.com/repos/siapi/siapi/releases/latest');
      const shouldUpdateSiapi = checkLatestSiapiVersion(siapiVersion, tag_name);

      getSiapiLatestReleaseSucceeded(tag_name, shouldUpdateSiapi);
      ...
      if (shouldUpdateSiapi) {
        siapi.notification.toggle({
          type: 'info',
          message: { id: 'notification.version.update.message' },
          link: {
            url: `https://github.com/siapi/siapi/releases/tag/${tag_name}`,
            label: {
  ...
  initApp = async () => {
    ...
    await this.fetchSiapiLatestRelease();
  };
    ...
    return (
      <PermissionsManager>
        <GlobalContextProvider
          ...
          shouldUpdateSiapi={shouldUpdateSiapi}
          plugins={plugins}
          settingsBaseURL={SETTINGS_BASE_URL || '/settings'}
          siapiVersion={siapiVersion}
...
Admin.propTypes = {
  admin: PropTypes.shape({
    ...
    shouldUpdateSiapi: PropTypes.bool.isRequired,
  }).isRequired,
  disableGlobalOverlayBlocker: PropTypes.func.isRequired,
  enableGlobalOverlayBlocker: PropTypes.func.isRequired,
  getInfosDataSucceeded: PropTypes.func.isRequired,
  getSiapiLatestReleaseSucceeded: PropTypes.func.isRequired,
