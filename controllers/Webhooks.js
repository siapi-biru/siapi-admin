...
const { yup, formatYupErrors, webhook: webhookUtils } = require('siapi-utils');
...
module.exports = {
  async listWebhooks(ctx) {
    const webhooks = await siapi.webhookStore.findWebhooks();
    ctx.send({ data: webhooks });
  },

  async getWebhook(ctx) {
    const { id } = ctx.params;
    const webhook = await siapi.webhookStore.findWebhook(id);
  ...
  async createWebhook(ctx) {
    ...
    const webhook = await siapi.webhookStore.createWebhook(body);

    siapi.webhookRunner.add(webhook);
  ...
  async updateWebhook(ctx) {
    ...
    const updatedWebhook = await siapi.webhookStore.updateWebhook(id, {
      ...webhook,
      ...body,
    });
    ...
    siapi.webhookRunner.update(updatedWebhook);
  ...
  async deleteWebhook(ctx) {
    ...
    await siapi.webhookStore.deleteWebhook(id);

    siapi.webhookRunner.remove(webhook);
  ...
  async triggerWebhook(ctx) {
    ...
    const response = await siapi.webhookRunner.run(webhook, 'trigger-test', {});
