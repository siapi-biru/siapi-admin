...
const deleteByRolesIds = async rolesIds => {
  const deletedPermissions = await siapi
...
const deleteByIds = async ids => {
  const deletedPermissions = await siapi.query('permission', 'admin').delete({ id_in: ids });
...
const createMany = async permissions => {
  const createdPermissions = await siapi.query('permission', 'admin').createMany(permissions);
...
const update = async (params, attributes) => {
  const updatedPermissions = await siapi.query('permission', 'admin').update(params, attributes);
...
const find = async (params = {}) => {
  const rawPermissions = await siapi.query('permission', 'admin').find(params, []);
...
const cleanPermissionsInDatabase = async () => {
  ...
  for (let page = 1; page * pageSize < total; page++) {
    ...
    const { pagination, results } = await siapi
      .query('permission', 'admin')
...
const ensureBoundPermissionsInDatabase = async () => {
  ...
  const contentTypes = Object.values(siapi.contentTypes);
  const editorRole = await siapi.query('role', 'admin').findOne({ code: EDITOR_CODE }, []);
