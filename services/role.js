...
const create = async attributes => {
  ...
  return siapi.query('role', 'admin').create(rolesWithCode);
};
...
const findOne = (params = {}, populate = []) => {
  return siapi.query('role', 'admin').findOne(params, populate);
};
...
const findOneWithUsersCount = async (params = {}, populate = []) => {
  const role = await siapi.query('role', 'admin').findOne(params, populate);
...
const find = (params = {}, populate = []) => {
  return siapi.query('role', 'admin').find(params, populate);
};
...
const findAllWithUsersCount = async (populate = []) => {
  const roles = await siapi.query('role', 'admin').find({ _limit: -1 }, populate);
...
const update = async (params, attributes) => {
  ...
  return siapi.query('role', 'admin').update(params, sanitizedAttributes);
};
...
const exists = async params => {
  const foundCount = await siapi.query('role', 'admin').count(params);
...
const count = async (params = {}) => {
  return siapi.query('role', 'admin').count(params);
};
...
const deleteByIds = async (ids = []) => {
  ...
  let deletedRoles = await siapi.query('role', 'admin').delete({ id_in: ids });
...
const getUsersCount = async roleId => {
  return siapi.query('user', 'admin').count({ roles: [roleId] });
};
...
const displayWarningIfNoSuperAdmin = async () => {
  ...
  if (!superAdminRole) {
    siapi.log.warn("Your application doesn't have a super admin role.");
  } else if (someUsersExists && superAdminRole.usersCount === 0) {
    siapi.log.warn("Your application doesn't have a super admin user.");
  }
};
...
const assignPermissions = async (roleId, permissions = []) => {
  try {
    ...
    throw siapi.errors.badRequest('ValidationError', err);
  }
