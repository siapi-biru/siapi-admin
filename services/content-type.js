  ...
  return actions.reduce((permissions, action) => {
    ...
    for (const subject of validSubjects) {
      const fields = actionDomain.appliesToProperty('fields', action)
        ? getNestedFields(siapi.contentTypes[subject], {
            components: siapi.components,
            nestingLevel,
          })
        : undefined;
...
const cleanPermissionFields = (permissions, { nestingLevel } = {}) => {
  ...
  return permissions.map(permission => {
    ...
    if (!subject || !siapi.contentTypes[subject]) {
      return permission;
    }

    const possibleFields = getNestedFieldsWithIntermediate(siapi.contentTypes[subject], {
      components: siapi.components,
      nestingLevel,
    });

    const requiredFields = getNestedFields(siapi.contentTypes[subject], {
      components: siapi.components,
      requiredOnly: true,
      nestingLevel,
